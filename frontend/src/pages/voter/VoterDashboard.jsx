...
import { Vote, CheckCircle, AlertCircle, ShieldCheck, Camera, ScanFace, Sun, Moon, LogOut, User, Search, PieChart } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const performAutoVerification = async () => {
      ...
      try {
        ...
        if (!hasBlinkedRef.current) {
          ...
          if (detectRes.data.detected) {
            if (detectRes.data.blinking) {
              ...
              setError('Liveness verified! Open your eyes for identity match...');
            } else {
              setError('Face detected! Please BLINK to verify liveness.');
            }
          } else {
            setError(detectRes.data.detail || 'Scanning... Position your face.');
          }
        ...
        autoVerifyRef.current = false;
        if (cameraActive) {
          timeoutId = setTimeout(performAutoVerification, 1000); // Try again
        }
      }
    };
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white transition-colors duration-300">
      ...
      <aside className="w-full md:w-72 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col shadow-lg md:sticky md:top-0 md:h-screen z-20">
        ...
        <nav className="flex-1 px-4 py-4 space-y-2 overflow-y-auto">
          ...
          <button 
            onClick={() => { setActiveTab('track'); setTrackingResult(null); setTrackingError(''); }} 
          ...
          {isAdmin && (
            <Link to="/admin" className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl font-bold text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all duration-200">
      ...
      <main className="flex-1 p-6 sm:p-10 lg:p-12 overflow-y-auto">
          ...
          <>
              ...
              <div className="grid gap-6 md:grid-cols-2">
                {isLoadingElections ? (
                  ...
                  elections.map((election) => (
                    <div key={election.id} className="bg-white dark:bg-gray-800 p-8 rounded-3xl border border-gray-200 dark:border-gray-700 hover:border-blue-500 dark:hover:border-blue-500 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl cursor-pointer group shadow-md" onClick={() => handleSelectElection(election)}>
                      <h3 className="text-xl font-bold mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{election.title}</h3>
                      <span className="text-xs font-mono bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-200 px-3 py-1 rounded-md">ID: {election.id}</span>
            ...
            {selectedElection && !receipt && (
              <div className="bg-white dark:bg-gray-800 p-8 sm:p-10 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-xl dark:shadow-2xl">
                ...
                <h2 className="text-3xl font-extrabold mb-8">Ballot: <span className="text-blue-600 dark:text-blue-400">{selectedElection.title}</span></h2>
                ...
                <div className="space-y-4 mb-8">
                  {candidates.map((candidate) => (
                    <label key={candidate.db_id} className={`flex items-center p-5 rounded-2xl border-2 cursor-pointer transition-all duration-200 ${selectedCandidateIndex === candidate.candidate_index ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-500 shadow-md' : 'bg-gray-50 dark:bg-gray-800/50 border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 hover:border-gray-300 dark:hover:border-gray-500'}`}>
                      ...
                      />
                      <span className="ml-4 text-lg font-medium">{candidate.name}</span>
                    </label>
                  ))}
                </div>
                ...
                {!cameraActive ? (
                  <button 
                    ...
                    className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-bold py-4 rounded-xl transition-all duration-200 active:scale-95 text-lg shadow-lg flex items-center justify-center gap-2"
                  >
                    <Camera size={24} /> Verify Face to Vote
                  </button>
                ) : (
                  <div className="flex flex-col items-center bg-gray-50 dark:bg-gray-900 p-8 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-inner">
                    <div className="mb-6 text-center">
                      <ScanFace className="w-10 h-10 text-blue-600 dark:text-blue-400 mx-auto mb-3 animate-pulse" />
                      <p className="text-base text-gray-700 dark:text-gray-300 font-bold">Auto-detecting face...</p>
                      <p className="text-sm text-gray-500 dark:text-gray-500 mt-1">Position your face inside the dashed circle</p>
                    </div>
                    <div className="relative w-full max-w-md mx-auto mb-6 overflow-hidden rounded-2xl border-2 border-gray-300 dark:border-gray-600 bg-black aspect-video shadow-lg">
                      <video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover transform scale-x-[-1]" />
                      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 h-3/4 aspect-square border-4 border-dashed border-blue-500/70 rounded-full pointer-events-none z-10 animate-pulse"></div>
                    </div>
                    <canvas ref={canvasRef} className="hidden" />
                    <div className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400 font-bold py-4 rounded-xl flex items-center justify-center gap-3 shadow-sm">
            ...
            {/* VIEW 3: RECEIPT */}
            {receipt && (
              <div className="bg-white dark:bg-gray-800 p-8 sm:p-12 rounded-3xl border border-green-500/50 shadow-2xl shadow-green-100 dark:shadow-green-900/20">
                <div className="text-center mb-10">
                  <CheckCircle className="w-20 h-20 text-green-500 mx-auto mb-6" />
                  <h2 className="text-4xl font-extrabold text-gray-900 dark:text-white mb-2">Vote Recorded!</h2>
                ...
                <div className="bg-gray-50 dark:bg-gray-900 p-6 sm:p-8 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-inner">
                  <div className="text-center">
                    ...
                    <span className="text-yellow-600 dark:text-yellow-400 select-all font-mono font-bold text-xl">{receipt.receipt_id}</span>
                  </div>
        ...
        {activeTab === 'track' && (
          <div className="bg-white dark:bg-gray-800 p-8 sm:p-10 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-xl dark:shadow-2xl">
            <h2 className="text-3xl font-extrabold mb-4">Track Your Vote</h2>
        ...
        {activeTab === 'results' && (
          <div className="space-y-6">
            <h2 className="text-3xl font-extrabold mb-8">Election Results</h2>
            {isLoadingResults ? (
              ...
              <div className="text-center py-10 text-gray-500 bg-gray-100 dark:bg-gray-800/50 rounded-3xl border border-gray-300 dark:border-gray-700 border-dashed">
                No published election results found.
              </div>
            ) : (
              pastResults.map(election => (
                <div key={election.id} className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-xl">
                  <div className="flex items-center gap-3 mb-6 border-b border-gray-200 dark:border-gray-700 pb-4">
                    ...
                    <h3 className="font-extrabold text-xl text-gray-900 dark:text-white">{election.title} <span className="text-gray-500 font-mono text-sm ml-2 font-semibold bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">ID: {election.id}</span></h3>
                  </div>
                  
                  <div className="flex flex-col md:flex-row items-center gap-10">
                    <div 
                      className="w-48 h-48 sm:w-56 sm:h-56 rounded-full shadow-lg border-4 border-white dark:border-gray-800 flex-shrink-0 transition-all duration-500"
                    ...
                    <div className="flex-1 w-full space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {Object.entries(election.official_results).map(([candidate, votes], index) => {
                          ...
                          return (
                            <div key={candidate} className="bg-gray-50 dark:bg-gray-900 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm flex items-center gap-4">
